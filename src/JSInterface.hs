...
errorToJsonString :: EC.Localizer -> String -> EC.Error -> String
errorToJsonString localizer source error =
  Char.unpack $ Json.encode $ EC.errorToJson localizer source error

makeErrorListJS :: EC.Localizer -> [EC.Error] -> String -> IO (JSRef a)
makeErrorListJS localizer errors source = do
  hsArray <- pure $ errors
    & map (errorToJsonString EC.dummyLocalizer source)
    & map JS.toJSString
...
compileInterfaceService requestChan replyChan =
  forever $ do
    ...
    let ifaces = map (\(CanonicalNameAndVersion cnv ver, intf) -> (cnv,intf)) ifacesCNV
    let res = EC.compile ctx source (Map.fromList ifaces)
